...
describe('DnGmxJuniorVaultManager', () => {
  ...
  describe('#getQuote', () => {
    it('wbtc to usdc exactIn', async () => {
      const { quoter, test } = await loadFixture(deployTest);

      const btcAmount = parseBtc('1');
      const usdcAmount = await test.getQuote(btcAmount, test.WBTC_TO_USDC(), test.USDC_TO_WBTC());
    ...
    it('usdc to wbtc exactIn', async () => {
      const { quoter, test } = await loadFixture(deployTest);

      const usdcAmount = parseUsdc('20000');
      const btcAmount = await test.getQuote(usdcAmount, test.USDC_TO_WBTC(), test.WBTC_TO_USDC());
    ...
    it('weth to usdc exactIn', async () => {
      const { quoter, test } = await loadFixture(deployTest);

      const ethAmount = parseEther('1');
      const usdcAmount = await test.getQuote(ethAmount, test.WETH_TO_USDC(), test.USDC_TO_WETH());
    ...
    it('usdc to weth exactIn', async () => {
      const { quoter, test } = await loadFixture(deployTest);

      const usdcAmount = parseUsdc('2000');

      const ethAmount = await test.getQuote(usdcAmount, test.USDC_TO_WETH(), test.WETH_TO_USDC());
