...
export const dnGmxJuniorVaultFixture = deployments.createFixture(async hre => {
  ...
  const quoter = await (
    await hre.ethers.getContractFactory('QuoterV3')
  ).deploy('0x1F98431c8aD98523631AE4a59f267346ea31F984', weth.address);
  ...
  await dnGmxJuniorVault.setParamsV1(BigNumber.from(2).pow(128).sub(1), quoter.address);
